...
fn get_num_energized(map: &Map, start_pos: (usize, usize), start_dir: Direction) -> u64 {
    let mut map = map.clone();
    traverse(&mut map, start_pos, start_dir);
    map.iter()
        .flatten()
...
pub fn func(path: &Path) -> u64 {
    ...
    let map: Map = read_to_string(path)
        .unwrap()
        .lines()
        .map(|s| s.chars().map(|c| (Tile::build(c).unwrap(), 0)).collect())
        .collect();
    let mut v = Vec::<u64>::with_capacity(map.len() * 2 + map[0].len() * 2);
    for i in 0..map.len() {
        v.push(get_num_energized(&map, (i, 0), Direction::Right));
        v.push(get_num_energized(
            &map,
            (i, map[0].len() - 1),
            Direction::Left,
        ));
    ...
    for i in 0..map[0].len() {
        v.push(get_num_energized(&map, (0, i), Direction::Down));
        v.push(get_num_energized(&map, (map.len() - 1, i), Direction::Up));
    }
    *v.iter().max().unwrap()
}
